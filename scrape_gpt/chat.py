...
load_dotenv()
tools = Tools(exclude_actions=["exclude_structured_data"])


class Link(BaseModel):
    information: str = Field(..., description="what subpage or item the link is pointing to")


class ExtractInfoInput(BaseModel):
    information_to_find: str = Field(..., description="what information to find on each subpage")
    subpage_links_to_extract: List[str] = Field(..., description="a single subpage link to extract information from")
...
async def extract_links_from_dom(subpage_to_find: str, browser_session: BrowserSession):
    ...
    try:
        ...
        scraper = SmartScraperGraph(
            prompt=f"Extract links leading to {subpage_to_find}. Use full URL not only relative path. Current url is " + current_url,
            source=page_html,
            config={
                "llm": {
        ...
        return ActionResult(extracted_content=success_msg, include_in_memory=True, long_term_memory=f'Extracted {subpage_to_find} links with number of {len(result["content"])}')
...
async def extract_current_page_info(information_to_find: str, browser_session: BrowserSession):
    """
...
async def main():
    agent = Agent(
        task="""
        go to https://medrecruit.medworld.com/jobs/list?location=New+South+Wales&page=1 use extract_info_from_subpages tool with param {subpage_links_to_extract: ['https://medrecruit.medworld.com/jobs/registrar/obstetrics-and-gynaecology/jn00306752'], information_to_find: 'job details'}
        """,
        llm=ChatOpenAI(model='gpt-4.1-mini'),
        tools=tools,  # Our custom tools ScrapeGraph
    )
    _ = await agent.run()
    print("history")
