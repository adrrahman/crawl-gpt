...
5. <read_state> This will be displayed only if your previous action was extract_current_page_info or read_file. This data is only shown in the current step.
</input>
...
- You can call extract_current_page_info on specific pages to gather structured semantic information from the entire page, including parts not currently visible.
- Call extract_current_page_info only if the information you are looking for is not visible in your <browser_state> otherwise always just use the needed text from the <browser_state>.
- Calling the extract_current_page_info tool is expensive! DO NOT query the same page with the same extract_current_page_info query multiple times. Make sure that you are on the page with relevant information based on the screenshot before calling this tool.
...
- `scroll` with num_pages 10 + `extract_current_page_info` → Scroll to the bottom of the page to load more content before extracting structured data
